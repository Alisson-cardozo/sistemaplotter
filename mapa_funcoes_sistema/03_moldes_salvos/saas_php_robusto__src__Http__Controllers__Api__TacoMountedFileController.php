...
final class TacoMountedFileController extends BaseController
{
    public function index(Request $request, array $params)
    {
        ...
        $sql = ($user['role'] ?? 'user') === 'admin'
            ? "SELECT arquivos_montados_taco.id, arquivos_montados_taco.nome_projeto AS nomeProjeto, arquivos_montados_taco.modelo, arquivos_montados_taco.quantidade_gomos AS quantidadeGomos, arquivos_montados_taco.comprimento_gomo_cm AS comprimentoGomoCm, arquivos_montados_taco.created_at AS createdAt, arquivos_montados_taco.payload_json AS payloadJson, COALESCE(users.name, 'Usuario removido') AS feitoPor FROM arquivos_montados_taco LEFT JOIN users ON users.id = arquivos_montados_taco.user_id ORDER BY arquivos_montados_taco.created_at DESC"
            : "SELECT arquivos_montados_taco.id, arquivos_montados_taco.nome_projeto AS nomeProjeto, arquivos_montados_taco.modelo, arquivos_montados_taco.quantidade_gomos AS quantidadeGomos, arquivos_montados_taco.comprimento_gomo_cm AS comprimentoGomoCm, arquivos_montados_taco.created_at AS createdAt, arquivos_montados_taco.payload_json AS payloadJson, COALESCE(users.name, 'Usuario removido') AS feitoPor FROM arquivos_montados_taco LEFT JOIN users ON users.id = arquivos_montados_taco.user_id WHERE arquivos_montados_taco.user_id = ? ORDER BY arquivos_montados_taco.created_at DESC";
    ...
    {
        $user = $this->user($request);
        $stmt = Db::pdo()->prepare('SELECT * FROM arquivos_montados_taco WHERE id = ? LIMIT 1');
        $stmt->execute([(int) ($params['id'] ?? 0)]);
        $item = $stmt->fetch();
        if (!is_array($item)) {
            return $this->fail('Arquivo montado nao encontrado.', 404);
        }
        if (($user['role'] ?? 'user') !== 'admin' && (int) ($item['user_id'] ?? 0) !== (int) $user['id']) {
            return $this->fail('Voce nao pode acessar este arquivo montado.', 403);
        }
    ...
    public function store(Request $request, array $params)
    {
        ...
        $fileName = trim((string) ($input['modelo'] ?? '')) !== ''
            ? trim((string) ($input['modelo'] ?? ''))
            : (trim((string) ($input['projeto'] ?? '')) !== '' ? trim((string) ($input['projeto'] ?? '')) : 'Sem nome');
        Db::pdo()->prepare('INSERT INTO arquivos_montados_taco (user_id, nome_projeto, modelo, quantidade_gomos, comprimento_gomo_cm, diametro_boca_cm, bainha_cm, payload_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')->execute([
            (int) $user['id'],
            $fileName,
        ...
        return $this->ok(['message' => 'Arquivo montado salvo com sucesso.'], 201);
    }
    ...
    public function destroy(Request $request, array $params)
    {
        ...
        if (($user['role'] ?? 'user') !== 'admin') {
            return $this->fail('Apenas o administrador pode excluir arquivos montados.', 403);
        }

        $stmt = Db::pdo()->prepare('DELETE FROM arquivos_montados_taco WHERE id = ?');
        $stmt->execute([(int) ($params['id'] ?? 0)]);
        ...
        return $this->ok(['message' => 'Arquivo montado excluido com sucesso.']);
    }
